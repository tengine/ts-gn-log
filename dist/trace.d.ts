...
export declare const INVALID_TRACE_ID: string;
export declare const INVALID_SPAN_ID: string;
/**
...
} | Map<string, unknown> | Readonly<Record<string, unknown>>;
/** 大文字小文字を区別せずにヘッダの値を取り出す。無ければ undefined */
export declare function headerValue(headers: HeadersLike, name: string): string | undefined;
/**
 * W3C の traceparent ヘッダを解釈する。undefined や不正な形式、trace_id / span_id が
 * すべて 0 の無効値は undefined を返す。
 */
export declare function parseTraceparent(value: string | undefined): TraceContext | undefined;
/**
 * W3C の traceparent ヘッダの値を組み立てる。traceparent は parent-id と flags が必須で
 * 「不明」を表せないため、spanId と sampled の両方が分かっているときだけ組み立てる。
 */
export declare function formatTraceparent(trace: TraceContext): string | undefined;
