...
もう 1 つの限界は孤立サロゲートです。対にならないサロゲート (JS 側で文字列を UTF-16 単位に切り詰めた結果 — `"boom 😀".slice(0, 6)` など — に現れます) は UTF-8 にエンコードできません。ts 側は U+FFFD に置き換えて値を返します (ログの呼び出しは投げない方針に合わせています)。py-gn-log と値が一致するかは、孤立サロゲートが正規化と切り詰めの後にも残る位置にあるかで決まります。切り詰めで落ちる位置にあれば影響せず、値は一致します。残る位置にあると、py 側は UTF-8 にエンコードできない文字列を扱うことになり、ts と同じ fingerprint が得られるとは限りません。**py 側で何が起きるか (例外になるか、ログの行が残るか) を前提にしないでください** — 形式・設定・出力先によって変わります。条件の一覧と扱いの決定は py-gn-log 側に委ねます (未決)。呼び出す前に ts 側でメッセージを切り詰めるなら、コードポイント単位で行ってください (UTF-16 コード単位で切ると孤立サロゲートを作ります)。

**入力の大きさの上限は呼び出し側の責務です。** 置換は入力の全体に走るので、`normalizeMessage` / `buildFingerprint` は入力の大きさに比例したメモリを使います (py-gn-log の `fingerprint.py` も同じ構造で、3 つの `sub` を全体にかけてから切り詰めます)。落ちるかどうかはヒープの大きさと、置換でどれだけ膨らむか次第です — 実測では、ヒープを 512MB に制限した環境で 40MB の数値を多く含むメッセージが `try`/`catch` で受けられない fatal OOM になり、既定のヒープ (約 4GB) では 40MB は通って 250MB 相当で落ちました。ログの経路では、PR 7 で `createLogger` の `errorEvent` を実装するときに上限を超えたメッセージへ fingerprint を付けないようにします (**この PR の時点では未実装**)。それまでと、これらの関数を直接呼ぶ場合は、呼び出し側でメッセージの長さを制限してください。
