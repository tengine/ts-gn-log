...
- **入力の大きさ**: 置換は入力の全体に走るので、入力の大きさに比例したメモリを使います。十分に大きな入力では `try`/`catch` で受けられない fatal OOM でプロセスが落ちます (落ちる大きさはヒープの設定次第。実測値は docstring にあります)。**上限は呼び出し側の責務です** (py-gn-log #35 で未決)。ログの経路では PR 7 の `errorEvent` が、正規化の前のメッセージが 10,000 コードポイントを超えたら fingerprint を付けずに行を出します (両言語の契約。この PR の時点では未実装)。
