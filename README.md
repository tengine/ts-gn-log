...
もう 1 つの限界は孤立サロゲートです。対にならないサロゲート (JS 側で文字列を UTF-16 単位に切り詰めた結果 — `"boom 😀".slice(0, 6)` など — に現れます) は UTF-8 にエンコードできません。ts 側は U+FFFD に置き換えて値を返します (ログの呼び出しは投げない方針に合わせています)。孤立サロゲートが正規化と切り詰めの後にも残る位置にあると、py 側は `UnicodeEncodeError` になるので両言語の値が揃いません (切り詰めで落ちる位置にあれば影響せず、値は一致します)。py 側でこの例外が fingerprint の計算と書き出しのどちらで起き、その行が残るかどうかは形式・設定・出力先によります — **py 側で行が残ることを前提にしないでください。** 条件の一覧と扱いの決定は py-gn-log 側に委ねます (未決)。ts 側で切り詰めるならコードポイント単位で行ってください。
