...
もう 1 つの限界は孤立サロゲートです。対にならないサロゲート (JS 側で文字列を UTF-16 単位に切り詰めた結果 — `"boom 😀".slice(0, 6)` など — に現れます) は UTF-8 にエンコードできません。ts 側は U+FFFD に置き換えて値を返すので (ログの呼び出しは投げない方針に合わせています) 両言語の値が揃わず、py 側は fingerprint の計算 (`error_event` 有効時) か書き出し (エンコードが走る経路) のいずれかで `UnicodeEncodeError` になります。**py 側でその行が残ることを期待しないでください** — 残るのは JSON 形式・`error_event` 無し・`json_ensure_ascii` 既定の組み合わせだけで、py で実行した他の組み合わせ (`json_ensure_ascii=False`、`error_event` 有効、text 形式) ではいずれも行が失われました。切り詰めるならコードポイント単位で行ってください。契約としてどちらに揃えるかは py-gn-log 側で決めます (未決)。
