...
- **孤立サロゲート (両言語で値が一致しない条件)**: 対にならないサロゲート (JS 側で文字列を UTF-16 単位に切り詰めた結果、たとえば `"boom 😀".slice(0, 6)` に現れます) は UTF-8 にエンコードできません。ts 側は Node の既定に従って U+FFFD に置き換えて値を返します (ログの呼び出しは投げない方針に合わせています)。py-gn-log と値が一致するかは、孤立サロゲートが正規化と切り詰めの後にも残る位置にあるかで決まります。切り詰めで削られる位置にあれば影響せず、両言語の値は一致します (2026-09-14 に `buildFingerprint("w", "op", "t", "x".repeat(400) + "\uD83D")` と、py-gn-log の同じ引数の `build_fingerprint` を実行し、どちらも `ba925d5a90b03dee` になることを確認しました。値は surface / operation / error_type によって変わります。この一致を検査する関門はありません)。残る位置にあると、py 側は UTF-8 にエンコードできない文字列を扱うことになり、ts と同じ fingerprint が得られるとは限りません。**py 側で何が起きるか (例外になるか、ログの行が残るか) は前提にしないでください** (py-gn-log #34)。呼び出す前に ts 側でメッセージを切り詰めるなら、コードポイント単位で行ってください (UTF-16 コード単位で切ると孤立サロゲートを作ります)。
- **入力の大きさ (呼び出し側の責務)**: 置換は入力の全体に走るので、`normalizeMessage` / `buildFingerprint` は入力の大きさに比例したメモリを使います。落ちるかどうかはヒープの大きさと置換でどれだけ膨らむか次第で、実測では、ヒープを 512MB に制限した環境で 40MB の数値を多く含むメッセージが `try`/`catch` で受けられない fatal OOM になり、既定のヒープ (約 4GB) では 40MB は通って 250MB 相当で落ちました。**上限は呼び出し側の責務です** (py-gn-log #35)。ログの経路での上限は[設計案 §2.5 fingerprint を付けない入力の上限](docs/designs/0001-ts-gn-log-design.md#25-fingerprint-を付けない入力の上限)を参照してください。
