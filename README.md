...
- **孤立サロゲート (両言語で値が一致しない条件)**: 対にならないサロゲート (JS 側で文字列を UTF-16 単位に切り詰めた結果、たとえば `"boom 😀".slice(0, 6)` に現れます) は UTF-8 にエンコードできません。ts 側は Node の既定に従って U+FFFD に置き換えて値を返します (ログの呼び出しは投げない方針に合わせています)。py-gn-log と値が一致するかは、孤立サロゲートが正規化と切り詰めの後にも残る位置にあるかで決まります。切り詰めで削られる位置にあれば影響せず、両言語の値は一致します (2026-09-14 に `"x".repeat(400)` の後ろに孤立サロゲートを置いた入力で両言語を実行し、どちらも `ba925d5a90b03dee` になることを確認しました。この一致を検査する関門はありません)。残る位置にあると、py 側は UTF-8 にエンコードできない文字列を扱うことになり、ts と同じ fingerprint が得られるとは限りません。**py 側で何が起きるか (例外になるか、ログの行が残るか) は前提にしないでください** (py-gn-log #34 で未決)。呼び出す前に ts 側でメッセージを切り詰めるなら、コードポイント単位で行ってください (UTF-16 コード単位で切ると孤立サロゲートを作ります)。
